...
The page links to no downloadable files: print files and the display model are not offered. The render JPEGs are copied from the verified v16 `Boom_Boom_complete.zip`, and the model is tied to that release's v16 3MF hash in `delivery_manifest.json`. Hashes and the part list are recorded in `assets/models/boom.provenance.json`. The script expects that v16 release; a newer kit revision needs its release file name updated in the script. The source folder is only read. Printed fit, glue strength and clip life are untested; the page says so.
