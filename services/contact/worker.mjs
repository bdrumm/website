...
export async function handle(request, env, send = fetch) {
  ...
  try {
    ...
    while(true){const {done,value}=await reader.read();if(done)break;size+=value.byteLength;if(size>MAX_BYTES){await reader.cancel();return reply({error:'Message too large'},413);}chunks.push(value);}
    const bytes=new Uint8Array(size);let offset=0;for(const chunk of chunks){bytes.set(chunk,offset);offset+=chunk.length;}
    let data;try{data=JSON.parse(new TextDecoder().decode(bytes));}catch{return reply({error:'Invalid JSON'},400);}
    ...
    if(!name || name.length>120 || /[\r\n\x00]/.test(name) || email.length>254 || !EMAIL.test(email) || !message || message.length>5000 || !/^[a-f0-9-]{36}$/i.test(data.submissionId)) return reply({error:'Check your name, email, and message.'},400);
    // Fixed recipient and sender prevent the endpoint becoming an open mail relay.
    ...
    const result=await send('https://api.resend.com/emails',{method:'POST',headers:{Authorization:`Bearer ${env.RESEND_API_KEY}`,'Content-Type':'application/json','Idempotency-Key':`contact/${data.submissionId}/${hash}`},body:JSON.stringify(payload),signal:AbortSignal.timeout(10000)});
    if(!result.ok) return reply({error:'Unable to send right now. Please try again.'},502);
    const resultData=await result.json();if(!resultData.id) return reply({error:'Unable to confirm sending. Please try again.'},502);
    return reply({ok:true});
  } catch { return reply({error:'Unable to send right now. Please try again.'},503); }
}
