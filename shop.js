...
function detail(){const p=projects.find(p=>p.id===({arowana:'trout'}[new URLSearchParams(location.search).get('id')]||new URLSearchParams(location.search).get('id')));if(!p){empty('Project not found.','Choose a project from the project list.');root.append(link('All projects →','projects.html','secondary-button'));return;}
 ...
 if(p.id==='baguette-holder'){content.classList.add('baguette-detail');root.append(content);import('./src/baguette-project.js?v=bb53e81934').then(({buildBaguetteProject})=>buildBaguetteProject(content,p)).catch(()=>content.append(link('Open the working baguette preview →','reviews/baguette-v3/','secondary-button')));return;}
