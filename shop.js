...
import {buildStationStory} from './src/station-story.js?v=33d4baba10';
...
function catalogue(){
 ...
 for(const [index,p] of selected.entries()){
  ...
  if(p.experience==='fly-lab'){const preview=el('iframe');preview.src='fly-lab/walking-preview.html';preview.title='Fly walking automatically on a surface';preview.loading='lazy';preview.style.cssText='display:block;width:100%;height:280px;border:0;margin-bottom:22px';card.append(preview);
   ...
   import('./assets/model-viewer.js?v=2cc094c4d2')
...
function detail(){const p=projects.find(p=>p.id===({arowana:'trout','boom-boom':'boom'}[new URLSearchParams(location.search).get('id')]||new URLSearchParams(location.search).get('id')));if(!p){empty('Project not found.','Choose a project from the project list.');root.append(link('All projects →','projects.html','secondary-button'));return;}
 ...
 if(p.id==='boom'){content.classList.add('has-model');root.append(content);import('./src/boom-project.js?v=d3fab754e9').then(({buildBoomProject})=>buildBoomProject(root,content,p)).catch(()=>content.append(el('p','The Boom overview could not load. Please reload to try again.')));return;}
 ...
 if(p.modelUrl){
  ...
  import('./assets/model-viewer.js?v=2cc094c4d2').then(({mountModelViewer})=>mountModelViewer(viewer,p.modelUrl,p.title,p.action)).catch(()=>viewer.append(el('p','The 3D viewer is unavailable. Try reloading, or download the model below.')));
 ...
 if(p.modelUrl){info.prepend(el('h2',p.section==='hardware'?'Pebbl OS, within reach.':'The finished form'));info.append(link(p.section==='hardware'?'Download device model ↗':'Download colored model ↗',p.modelUrl,'secondary-button'),link('Enquire about this project →','mailto:info@parametric.space?subject='+encodeURIComponent(p.title+' enquiry'),'secondary-button'));}
 const shop=shopLink(p);if(shop)info.append(shop);content.append(info);root.append(content);
 if(p.features&&p.id!=='station'){const features=el('section',undefined,'hardware-features');features.setAttribute('aria-label','Pebbl features');for(const [title,copy] of p.features){const card=el('article');card.append(el('h2',title),el('p',copy));features.append(card);}root.append(features);}
 if(p.specs){const specs=el('section',undefined,'hardware-specs');specs.append(el('h2','Inside Pebbl'));const list=el('dl');for(const [name,value] of p.specs){const row=el('div');row.append(el('dt',name),el('dd',value));list.append(row);}specs.append(list);root.append(specs);}
