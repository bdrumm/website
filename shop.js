...
function catalogue(){
 ...
 for(const [index,p] of selected.entries()){
  ...
  if(p.experience==='fly-lab'){const preview=el('iframe');preview.src='fly-lab/walking-preview.html';preview.title='Fly walking automatically on a surface';preview.loading='lazy';preview.style.cssText='display:block;width:100%;height:280px;border:0;margin-bottom:22px';card.append(preview);
   ...
   import('./assets/model-viewer.js?v=db6594e584')
...
function detail(){const p=projects.find(p=>p.id===({arowana:'trout'}[new URLSearchParams(location.search).get('id')]||new URLSearchParams(location.search).get('id')));if(!p){empty('Project not found.','Choose a project from the project list.');root.append(link('All projects →','projects.html','secondary-button'));return;}
 ...
 if(p.id==='baguette-holder'){content.classList.add('baguette-detail');root.append(content);import('./src/baguette-project.js?v=8e711ec40f').then(({buildBaguetteProject})=>buildBaguetteProject(content,p)).catch(()=>content.append(link('Open the working baguette preview →','reviews/baguette-v3/','secondary-button')));return;}
 if(p.id==='station'){root.append(content);import('./src/station-project.js?v=c726de2b4a').then(({buildStationProject})=>buildStationProject(root,content,p)).catch(()=>content.append(el('p','The Station viewer is unavailable. Please reload to try again.')));return;}
 ...
 if(p.modelUrl){
  ...
  import('./assets/model-viewer.js?v=db6594e584').then(({mountModelViewer})=>mountModelViewer(viewer,p.modelUrl,p.title,p.action)).catch(()=>viewer.append(el('p','The 3D viewer is unavailable. Try reloading, or download the model below.')));
