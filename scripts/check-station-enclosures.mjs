...
assert.deepEqual(release.variants.map(v=>v.id),['p43','q43']);
assert.equal(data.variants.length,22);assert.equal(data.variants.filter(v=>!v.archive).length,2);
for(const v of data.variants){
 ...
 if(!v.archive){assert.deepEqual(v.hardware.battery_casing_mm,[65,36,10]);assert.deepEqual(v.hardware.speaker_mm,[26,26,5]);assert.ok(!v.parts.bezel);assert.ok(v.integralLip);assert.equal(v.integralLip.side_overlap_mm,.6);assert.equal(v.parameters.usb_boot_height,10);}
}
