# Baguette holder V3.18 — engineering review

V3.18 restores the compact rounded latch and removes the elongated recess and stationary infill patch introduced by V3.17. The new 45° cut is confined to the grip material projecting beyond the actual outer shell. The short spring, rounded corners, central retaining hook, fingernail inset and 2° inward set remain. The former patch region is checked against the uninterrupted pre-ramp shell. Physical fit, release force and durability remain untested.
...
A second scoop is cut into the underside of the projecting grip itself. Its elliptical tool is 10 mm wide and rises 0.6 mm into the grip. A 4 mm wide, 0.6 mm deep gauge can enter upward from below, while checked material remains above the recess and in front of it as a retaining rim. The central hook, grip and retaining shoulder remain, with small fillet reconstruction differences bounded by the geometry review. The new visible bevel is confined to the outward-projecting grip.
...
Four swept slot gauges check for leftover band bridges along the two straight sides of each compact latch.
...
Each half prints connection-down: +Y rises on B and −Y rises on A. The projecting grip has a planar 45° connection-facing lead-in, clipped only from material beyond the original outer shell. The compact flush spring and straight slot sides retain their rounded outline. The internal hook and receiver retain their earlier 45° lead-ins. The two fixed slot edges remain stationary during release, with at least 0.50 mm measured clearance to the moving tongue. CAD and preview checks verify this separation.

A separate support-independent check measures the exterior grip lead-in in the unloaded print geometry. Its scope excludes the flush spring and hidden retaining features. This follows the latest request to keep the angled cut on the projecting portion; it does not establish support-free printing of the entire latch. Actual sliced layers and the retained support settings are reviewed separately.
...
Final offline estimates are **A: 14h 35m 44s / 486.39 g**, **B: 14h 31m 20s / 486.24 g**, about **29.1 hours and 973 g** total before coupons. Exact STL, settings and G-code hashes are recorded in `review.json` and `slice-wall-review.json`.
