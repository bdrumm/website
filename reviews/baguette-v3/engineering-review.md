# Baguette holder V3.16 — engineering review

V3.16 extends the 45° connection-facing bevel across the entire latch tongue edge and its hook. This removes the flat downward-facing tongue end left by the previous hook-only bevel. The 2° nominal inward set (0.657 mm at the free tip), seated preload, central locking shoulder and fingernail grip remain. The central grip and retaining profiles, along with all geometry outside the latch regions, are compared against V3.15. Physical fit, release force and durability remain untested.
...
A second scoop is cut into the underside of the projecting grip itself. Its elliptical tool is 10 mm wide and rises 0.6 mm into the grip. A 4 mm wide, 0.6 mm deep gauge can enter upward from below, while checked material remains above the recess and in front of it as a retaining rim. The central hook and grip profiles are unchanged from V3.15. The new bevel spans the connection-facing end of the tongue, root reinforcement and hook.
...
Each half prints connection-down. The bevel normals are measured after forming the inward set, with +Y as the build direction on B and −Y on A. The new full-thickness tongue plane is mirrored between halves and formed after the inward set. Its slope advances inward by 1 mm for each 1 mm rise from the connection face. It leaves the central engagement and fingernail inset intact. A direct face-area check guards against a remaining flat downward-facing tongue end below the root transition. The existing grip and receiver bevels are retained. The stationary U-slot edge follows the same slope with 0.45 mm normal clearance, restoring only the original shell material beside the bevel on both sides of the case seam. This also closes the newly exposed side of the catch pocket. Separate CAD and preview checks verify that this rim stays fixed when the tongue flexes. This is an edge bevel across thickness; the latch retains its original spring length.
...
Final offline estimates are **A: 14h 39m 43s / 487.06 g**, **B: 14h 32m 55s / 486.16 g**, about **29.2 hours and 973 g** total before coupons. Exact STL, settings and G-code hashes are recorded in `review.json` and `slice-wall-review.json`.
