# Baguette holder V3.17 — engineering review

V3.17 replaces the steep connection-facing latch outline with a continuous ramp from the shell attachment to the free tip. Separate sloped transitions grow the grip outward and the hook inward. The central locking shoulder, fingernail inset and 2° inward set remain. The leading root footprint is extended to make room for the ramp; all geometry outside the latch sites is compared against V3.16. Physical fit, release force and durability remain untested.
...
A second scoop is cut into the underside of the projecting grip itself. Its elliptical tool is 10 mm wide and rises 0.6 mm into the grip. A 4 mm wide, 0.6 mm deep gauge can enter upward from below, while checked material remains above the recess and in front of it as a retaining rim. The central hook, grip and retaining shoulder remain, with small fillet reconstruction differences bounded by the geometry review. The new ramp spans the leading tongue outline, with additional sloped transitions into the hook and grip.
...
Four swept slot gauges check for leftover band bridges along both sides of both latches, following the diagonal edge on the connection-facing side.
...
Each half prints connection-down: +Y rises on B and −Y rises on A. The front-profile ramp advances 1 mm toward the free tip for every 1.6 mm of build rise. Its measured leading overhang is at most 32.05° from vertical, below the requested 45° maximum. The outward grip and inward hook have additional sloped lead-ins; the receiver retains its 45° face. The stationary slot edge follows the tongue with 0.45 mm normal clearance. CAD and preview checks verify that the rim stays fixed during release.

A separate support-independent check samples 208 actual CAD cross-sections per half at 0.20 mm spacing, from the start of the tongue through 2 mm before the central catch. Each tongue section must lie within the preceding full-lid section expanded by one layer height. Both halves pass, with no leading face above 45° and no unsupported growth beyond numerical residue. This checks the requested leading ramp; retaining undercuts and other case features still need their own slicer support review.
...
Final offline estimates are **A: 14h 45m 33s / 485.35 g**, **B: 14h 41m 43s / 485.11 g**, about **29.5 hours and 970 g** total before coupons. Exact STL, settings and G-code hashes are recorded in `review.json` and `slice-wall-review.json`.
