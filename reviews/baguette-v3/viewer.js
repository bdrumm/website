...
export async function mountReview(root, buffer) {
 ...
 const presets={overall:{target:[0,0,-20],position:[120,180,680],width:350},hinge:{target:[150,0,-41],position:[190,150,10],width:32},section:{target:[150,0,-41],position:[50,2,-44],width:15},latch:{target:[150,0,43],position:[170,30,225],width:35},catch:{target:[150,-4,42],position:[245,5,45],width:18},joint:{target:[-4,0,0],position:[28,80,190],width:85}};
 ...
 function pose(){const degrees=Number(angle.value);hinge.rotation.x=-T.MathUtils.degToRad(degrees);angleValue.textContent=degrees+'°';
  for(const {mesh,rest,color} of meshes){mesh.visible=mesh.userData.reviewSection?view==='section':view!=='section';mesh.position.copy(rest);if(mesh.name.endsWith('_A'))mesh.position.x-=exploded.checked?58:0;else if(mesh.name.endsWith('_B'))mesh.position.x+=exploded.checked?58:0;
   ...
   mesh.material.clippingPlanes=cutaway.checked?(view==='section'?[]:view==='catch'?[new T.Plane(new T.Vector3(-1,0,0),150),new T.Plane(new T.Vector3(1,0,0),-136),new T.Plane(new T.Vector3(0,0,1),-30)]:[new T.Plane(new T.Vector3(0,-1,0),0)]):[];
  }
  status.textContent=view==='section'?'Blue: base / pin · Gold: lid · '+degrees+'°':degrees>0?'Latches shown released · '+degrees+'° open':(view==='catch'?'Blue: catch · Gold: latch':'Closed assembly');
 }
 function select(name){if(['catch','section'].includes(view)&&name!==view)cutaway.checked=false;view=name;cutaway.disabled=name==='section';exploded.disabled=name==='section';if(name==='section'){cutaway.checked=true;exploded.checked=false;}if(name==='catch'){angle.value='0';cutaway.checked=true;}pose();const p=presets[name];controls.target.fromArray(p.target);camera.position.fromArray(p.position);camera.zoom=1;halfWidth=p.width;root.querySelectorAll('[data-view]').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.view===name)));resize();controls.update();}
 root.querySelectorAll('[data-view]').forEach(b=>b.addEventListener('click',()=>{if(b.dataset.view==='hinge'&&Number(angle.value)===0){angle.value=String(maxAngle);pose();}if(b.dataset.view==='latch'){angle.value='0';pose();}select(b.dataset.view);}));
