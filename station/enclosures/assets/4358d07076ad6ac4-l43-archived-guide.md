# L43 — Batteryless switch, measured 4.3-inch board

**Measurement prototype — physical fit is not yet approved.**

Case, portrait: **69.3 × 132.3 × 17.6 mm**. Main rim: **16.4 mm**. Mount face to front rim: **26.9 mm**. Existing wall-cover projection is additional. Print the case and holder as a matched pair; M43/L43 holders are not interchangeable.
...
This revision is for the PCB labeled **ESP32-P4-WIFI6-Touch-LCD-4.3**. The owner measured the glass at **114 × 65 mm** and front glass to screw end at **11.5 mm**, and confirmed a **92 × 50 mm** screw pattern. Speaker housing: **26 × 26 × 5 mm**, lead **50 mm**. No battery is installed in L43.
...
The glass cavity is **114.7 × 65.7 mm**, giving 0.35 mm clearance on each side. Print the **glass-fit-ring** and **mount-pattern-template** first. The flat post template checks spacing; use calipers to compare its glass outline with the actual glass-to-post offsets. The **fit-jig** also registers the glass rim and screw seats at their actual depth. If the glass or posts need force, stop and record the mismatch; do not scale the whole print or tighten screws to pull the screen into place.
...
- Use the supplied millimeter 3MF at 100% scale. Each contains two separate objects: the matching device case and holder. Electronics and cable envelopes are preview references only.
...
- Print the case rear face down, opening upward. Raised pods mean the surrounding back needs support. Print the holder wall face down, hooks upward.
...
3. Leave BAT and CAMERA unused. Keep the rear cable bay open; no battery is installed in L43.
...
5. Lower the complete display through the open front. Both USB sockets face down. Check that the glass sits freely and every factory post rests on its seat. The rim stands 0.4 mm beyond the front glass.
...
To open the device, disconnect USB, remove the four rear screws and lift the intact display only as far as its leads allow. On the battery version disconnect BAT as soon as accessible. Side control-button openings and rear camera are omitted; board buttons require removal/service access.
