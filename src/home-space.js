...
async function mountSpace(stage) {
  ...
  await Promise.allSettled(bodies.map(async body => {
    try {
      const gltf = await loader.loadAsync(body.project.previewModelUrl || `assets/models/home/${body.project.id}.glb${body.project.id==='station'?'?v=clean-shell-3':body.project.id==='baguette-holder'?'?v=contiguous-1':''}`);
