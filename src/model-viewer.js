...
import {installBaguetteStudio} from './baguette-studio.js';
...
export async function mountModelViewer(host,modelUrl,title="3D print",actionKind="",options={}){
 ...
 const controls=new OrbitControls(camera,canvas);controls.enableDamping=true;controls.enablePan=false;controls.minDistance=7;controls.maxDistance=26;controls.target.set(.1,0,0);controls.autoRotate=!isStation&&actionKind!=='swim'&&!reducedMotion;controls.autoRotateSpeed=1.3;
 ...
 try{const gltf=await new GLTFLoader().loadAsync(modelUrl+(modelUrl.includes('?')?'&':'?')+'v='+(actionKind==='station'?'station-clean-sway-4':actionKind==='open'?'baguette-contiguous-1':'display-edge-2'));const bounds=new THREE.Box3().setFromObject(gltf.scene);const center=bounds.getCenter(new THREE.Vector3());const size=bounds.getSize(new THREE.Vector3());const scale=8/Math.max(size.x,size.y,size.z);gltf.scene.position.copy(center).multiplyScalar(-scale);gltf.scene.scale.setScalar(scale);scene.add(gltf.scene);modelAction=installAction(gltf.scene,actionKind);if(actionKind==='open'){experience=createBaguetteExperience(gltf.scene);loadButton.disabled=false;carryButton.disabled=false;}if(actionButton)actionButton.disabled=false;status.textContent=actionActive?'Swimming · Drag to explore':'Drag to rotate · Scroll or pinch to zoom';if(actionKind==='station'){const pmrem=new THREE.PMREMGenerator(renderer);const room=new RoomEnvironment();stationEnvironment=pmrem.fromScene(room,.04);scene.environment=stationEnvironment.texture;room.dispose();pmrem.dispose();resize();stationExperience=await installStation(gltf.scene,button,status,stopOrbit,host);pause.textContent=reducedMotion?'Resume motion':'Pause motion';pause.setAttribute('aria-pressed',String(!reducedMotion));controls.addEventListener('start',()=>stationExperience.setInteracting(true));controls.addEventListener('end',()=>stationExperience.setInteracting(false));}}catch{status.textContent='The model could not load. Try reloading or downloading the model.';observer.disconnect();baguetteStudio?.dispose();renderer.dispose();stage.remove();toolbar.remove();return;}
